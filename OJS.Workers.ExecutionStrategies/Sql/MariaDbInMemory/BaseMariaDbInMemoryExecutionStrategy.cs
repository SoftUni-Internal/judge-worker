namespace OJS.Workers.ExecutionStrategies.Sql.MariaDbInMemory
{
    ...
    public abstract class BaseMariaDbInMemoryExecutionStrategy : BaseSqlExecutionStrategy
    {
        ...
        protected BaseMariaDbInMemoryExecutionStrategy(
        ...
        public string RestrictedUserId => $"{this.restrictedUserId}_{this.GetDatabaseName()}".ToLower();
...
SET FOREIGN_KEY_CHECKS=1;";
        ...
        {
            commandText = base.FixCommandText(commandText);
            commandText = this.FixCreateTableQuery(commandText);
            commandText = this.FixTextColumn(commandText);
            return commandText;
        }

        private string FixTextColumn(string commandText)
            => Regex.Replace(commandText, "text not null", "varchar(16000) not null", RegexOptions.IgnoreCase);

        private string FixCreateTableQuery(string commandText)
        {
            const string addEngineInMemoryRegex =
                "Create table\\s\\w*?\\s?\\((?>\\((?<c>)|[^()]+|\\)(?<-c>))*(?(c)(?!))\\)";
            return Regex.Replace(
                commandText,
                addEngineInMemoryRegex,
                (match) => $"{match.Value} engine=memory",
