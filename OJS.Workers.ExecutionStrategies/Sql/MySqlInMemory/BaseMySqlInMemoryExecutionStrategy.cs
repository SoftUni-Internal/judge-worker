namespace OJS.Workers.ExecutionStrategies.Sql.MySqlInMemory
{
    ...
    public abstract class BaseMySqlInMemoryExecutionStrategy : BaseSqlExecutionStrategy
    {
        ...
        private static readonly string DatabaseName = $"{Guid.NewGuid()}";

        protected BaseMySqlInMemoryExecutionStrategy(
        ...
        public string RestrictedUserId => $"{this.restrictedUserId}_{this.GetDatabaseName().Replace("-", "")}".ToLower().Substring(0, 32);
        ...
        private void EnsureDatabaseIsSetup()
        {
            ...
            using (var connection = new MySqlConnection(this.sysDbConnectionString))
            {
                ...
                var createUserQuery = $@"
                    ...
                    SET PASSWORD FOR '{this.RestrictedUserId}'@'%'='{this.restrictedUserPassword}'";
        ...
        public override void DropDatabase(string databaseName)
        {
            ...
            using (var connection = new MySqlConnection(connectionString))
            {
                ...
                var dropAlLQuery = string.Join("\n",
                    ...
                    this.BuildDropProceduresQuery(connection, databaseName),
                    this.BuildDropFunctionsQuery(connection, databaseName));
                this.ExecuteNonQuery(connection, dropAlLQuery);
            }
        }

        private string BuildDropTableItemsByTypeQuery(IDbConnection connection, string databaseName, string type)
        {
            var getNamesQuery = $@"
SELECT table_name
...
SET FOREIGN_KEY_CHECKS=1;";
        ...
        private string BuildDropTablesQuery(IDbConnection connection, string databaseName)
            => this.BuildDropTableItemsByTypeQuery(connection, databaseName, "table");

        private string BuildDropViewsQuery(IDbConnection connection, string databaseName)
            => this.BuildDropTableItemsByTypeQuery(connection, databaseName, "view");

        private string BuildDropProceduresQuery(IDbConnection connection, string databaseName)
            => this.BuildDropRoutineItemsByTypeQuery(connection, databaseName, "procedure");

        private string BuildDropFunctionsQuery(IDbConnection connection, string databaseName)
            => this.BuildDropRoutineItemsByTypeQuery(connection, databaseName, "function");

        private string BuildDropRoutineItemsByTypeQuery(IDbConnection connection, string databaseName, string type)
        {
                var getNamesQuery = $@"
SELECT ROUTINE_NAME
FROM information_schema.ROUTINES
WHERE ROUTINE_SCHEMA = '{databaseName}'
    AND ROUTINE_TYPE LIKE UPPER('{type}');";

                var dropAllQuery = string.Join(
                    ";\n",
                    this.ExecuteReader(connection, getNamesQuery)
                        .Results
                        .Select(routineName => $"DROP {type} {routineName}"));
